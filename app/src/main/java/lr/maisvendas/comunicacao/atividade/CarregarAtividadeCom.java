...
public class CarregarAtividadeCom extends AsyncTask<String,Void,Response<List<Atividade>>> {

        private AtividadeServico service;
        private CarregarAtividadeTaskCallBack delegate;
        private String TAG = "CarregarAtividadeCom";
        private Ferramentas ferramentas;

    public CarregarAtividadeCom(CarregarAtividadeTaskCallBack delegate) {
        ...
        protected Response<List<Atividade>> doInBackground(String... params) {
            ...
            try {
                ...
                return carregarAtividadeCall.execute().body();
            } catch (Exception e) {
        ...
        protected void onPostExecute(Response<List<Atividade>> Atividade) {
            ...
            if (Atividade.getStatus().toString().equals("SUCCESS")) {
                delegate.onCarregarAtividadeSuccess(Atividade.getData());
            } else {
                delegate.onCarregarAtividadeFailure(Atividade.getMessage());
            }
        }

        public interface CarregarAtividadeTaskCallBack {
