...
public interface ProdutoServico {
    ...
    public Call<Response<Produto>>editarProduto(@Header("X-Auth-Token") String token, @Query("id") Integer id, @Body Produto produto);

    @DELETE("Produto/{id}/") //Deleta (não utilizado no produto)
    public Call<Response<Boolean>>excluirProduto(@Header("X-Auth-Token") String token, @Query("id") Integer id);
}
